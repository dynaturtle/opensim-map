...
//		llinfos << "Generating profile with LOD < MIN_LOD.  CLAMPING" << llendl;
	...
	if (begin > end - 0.01f)
	{
		MessageBox(0, L"LLProfile::generate() assertion failed (begin >= end)", 0, 0);
		return FALSE;
	}
...
//生成path、profile的各结点，进而变成mMesh[]中的顶点坐标
...
//	llassert_always(mProfilep);
	...
	//生成profile的各结点，以及Profile::mFaces(index, count...)
